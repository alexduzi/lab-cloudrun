...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -installsuffix cgo -ldflags="-w -s" -o /app/bin/api cmd/api/main.go
...
COPY --from=builder --chown=appuser:appuser /app/bin/api .
